...
def getRealArea(idx): # 미생물 체크 여부 확인 및 사망 처리
    ...
    #             real[data[idx][i][j]] += 1
    for i in range(q+1):
        if orgCheck[i] != 1: #BFS로 측정한 넓이와 실제 넓이가 다르면?
            clearDeath(idx,i)
...
def clearDeath(idx,num): # 죽거나 미확인 미생물 제거
    for i in range(n):
        for j in range(n):
            if data[idx][i][j] == num:
                data[idx][i][j] = 0
...
def bfs(idx, num, x, y): # BFS를 활용하여 데이터 갱신
    que = deque([(x,y)]) 
    check[x][y] = 1
    r1,c1,r2,c2 = n, n, 0, 0
    ex = 0
    ...
    for i in range(n):
        for j in range(n):
            ...
            if findArea(idx,i,j,alive):
                ...
                return
    _, num, r1,c1,r2,c2 = alive # 자리 못 찾았으면 사망
    orgs[num] = []
 
def findArea(idx,x,y,alive): # 배치 가능한 영역인지 체크
    _, num, r1, c1, r2, c2 = alive
    wid, hei = r2-r1, c2-c1
    if x+wid > n or y+hei > n: # 범위 바깥이면 False
        return False
    
    for i in range(wid): 
        for j in range(hei):
            if data[idx-1][r1+i][c1+j] == num: # 절대 좌표 기준으로 옮기려는 미생물이 존재하는 위치에
                if data[idx][x+i][y+j] and data[idx][x+i][y+j] != num: # 다른 미생물이 있다면
                    return False # 옮길 수가 읎어용
    return True # 전부 통과 됐으면 True
                             

def drawOrg(idx, x, y, alive): # 좌표에 맞게 미생물 배치
    _, num, r1, c1, r2, c2 = alive
    # print(alive)
...
def getResult(idx): # 모든 미생물에 대해 BFS를 진행하여 쌍 체크 진행
                ...
                while len(que) > 0:
                    ...
                    for a,b in [(0,1),(0,-1),(1,0),(-1,0)]:
                        ...
                        if 0 <= nx < n and 0 <= ny < n:
                            p = data[idx][nx][ny]
                            if p and o != p and not duoCheck[o][p]: # 체크되지 않은 쌍이라면 결과에 합산
                                duoCheck[o][p] = 1
                                duoCheck[p][o] = 1
