...
BFS인가? 우선순위 큐인가? 하면서 풀다가 둘 다 쓰는 문제임을 깨달았다...
상어의 현재 위치에서 방문할 수 있는 모든 경우의 수를 체크하는 BFS를 돌면서
먹을 수 있는 물고기의 거리와 위치를 모두 우선순위 큐에 박은 뒤
최우선 순위의 물고기를 먹는다. 그리고 이걸 반복하며 우선순위 큐에 아무것도 없으면 종료