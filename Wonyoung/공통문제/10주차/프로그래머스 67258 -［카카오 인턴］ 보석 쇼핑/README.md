...
### 문제 풀이 - 투 포인터 & 슬라이딩 윈도우

모든 종류의 보석을 포함하는 구간을 완전 탐색($O(N^2)$)으로 찾으면 진열대 길이가 최대 100,000이므로 효율성 테스트에서 100% 터집니다. 따라서 화살표 두 개(`start`, `end`)를 이용해 $O(N)$ 만에 끝내는 **투 포인터** 알고리즘을 적용합니다.



#### 1. 전체 보석 종류 파악 (`HashSet`)

- 진열대에 굴러다니는 총 보석 종류의 개수(`gemSet.size()`)를 먼저 구합니다.



#### 2. 최초의 유효 구간 확보

- `gemSet`에서 요소를 하나씩 지워가며 모든 종류의 보석이 최소 1개 이상 맵(`gemMap`)에 채워질 때까지 `end` 포인터를 전진시킵니다.



#### 3. 구간 축소 및 슬라이딩 (`Two Pointers`)

- **`start` 당기기 (구간 축소)**: 현재 맵에 잡힌 구간 중 맨 앞 보석(`gems[start]`)의 개수가 2개 이상이라면, 하나를 빼고 `start++`를 해도 모든 종류를 포함한다는 조건이 유지됩니다. 더 이상 줄일 수 없을 때까지(개수가 1개 남을 때까지) `start`를 전진시킵니다.
- **최솟값 갱신**: 현재 구간의 길이(`end - start`)가 기존 `range`보다 짧다면 정답 배열을 최신화합니다.
- **`end` 밀기 (구간 확장)**: `start`를 다 줄였다면 다음 보석을 탐색하기 위해 `end++`를 하고 새로운 보석을 맵에 추가합니다.