...
### 풀이 방법 - 크루스칼 알고리즘 + 유니온-파인드

가장 비용이 적은 다리부터 차례대로 탐색하며, 사이클을 만들지 않는 다리만 선택하여 모든 섬을 연결하는 **탐욕법(Greedy)** 기반의 알고리즘입니다.

#### 1. 간선(다리) 비용 기준 오름차순 정렬

- 모든 다리를 비용이 저렴한 순서대로 정렬합니다. (필기 예시: `PriorityQueue` 또는 배열 정렬 활용)

#### 2. 하나씩 연결할 때마다 "이미 이어졌는가" 체크 (`Union-Find`)

- 다리를 놓으려는 두 섬이 이미 연결되어 있는지 확인하여 **사이클(Cycle)** 발생을 방지합니다.
- **핵심 (대장 찾기)**: 각 노드의 대표 부모(Root)를 찾는 `find` 연산과, 두 집합을 합치는 `union` 연산을 사용합니다.
- **주의**: 트리가 분해되거나 사이클이 생기지 않도록 졸개가 아닌 대장 노드끼리 연결(`parent[B] = A`)해야 합니다.

#### 3. 모든 섬이 이어지면 총비용 합을 반환

- 간선의 개수가 `n - 1`개가 되면 모든 섬이 하나로 이어지므로 탐색을 조기 종료하고 최적의 비용을 리턴합니다.