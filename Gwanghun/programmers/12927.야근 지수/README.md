...
### 문제 풀이 방법
제일 큰 값을 기준으로 하나씩 빼주면서 문제를 해결하면 된다. (제곱은 늘 숫자가 클수록 제곱합이 더 커지는 구조이기 때문)
우선순위 큐 이용하기. 우선순위 큐는 기본적으로 제일 큰 값을 기준으로 정렬해주는 자료구조이다. (잊어버려서 답지 깜...)

| 연산    | 시간복잡도 | 설명 |
|--------|-----------|------|
| push() | O(log n)  | 삽입 후 힙 구조를 유지하기 위해 위로 이동 (heapify up) |
| pop()  | O(log n)  | 루트 제거 후 힙 구조를 유지하기 위해 아래로 이동 (heapify down) |
| top()  | O(1)      | 루트(최댓값 또는 최솟값) 반환 |
| size() | O(1)      | 원소 개수 반환 |
| empty()| O(1)      | 비어있는지 확인 |
